...
public class SingleDateAndTimePicker extends LinearLayout {
    ...
    private static final CharSequence FORMAT_12_HOUR = "EEE d MMM h:mm aa";
        ...
        picker.postDelayed(new Runnable() {
            ...
            public void run() {
                if (minDate != null && isBeforeMinDate(getDate())) {
                    if (daysPicker.getCurrentItemPosition() == 364) {
                        Calendar calendar = Calendar.getInstance();
                        calendar.setTime(minDate);
                        if (calendar.get(Calendar.HOUR_OF_DAY) >= WheelHourPicker.MAX_HOUR_AM_PM) {
                            amPmPicker.scrollTo(1);
                        } else {
                            amPmPicker.scrollTo(0);
                        }
    ...
    public void setMustBeOnFuture(boolean mustBeOnFuture) {
        ...
        if (mustBeOnFuture) {
            ...
            minDate = calendar.getTime(); //minDate is Today
        }
    }
